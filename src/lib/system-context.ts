...
export const CPUAGEN_KNOWLEDGE = [
  ...
  "CPUAGEN enforces quality through 9 independent safety barriers (Control Barrier Functions) that run on every message. ALL must pass. The barriers measure: Truth Alignment (BNR), Naturality (BNN), Energy Bounds (BNA), Thermal Stability (TSE), Coherence (PCD), Optimization Guard (OGP), Quality Metric (ECM), Synergy (SPC), and Free Energy Principle (FEP). These are mathematical functions computed from the physics of the response itself.",
  "",
...
export function getChatContext(): string {
  return [
    ...
    "Step 1: You type a message. Step 2: CPUAGEN's enforcement engine receives your message BEFORE the LLM sees it. Step 3: Your message is converted into a thermosolve signature — a physics-based representation. Step 4: The full Control Barrier series runs on your input (pre-validation). Step 5: If pre-validation passes, your message is forwarded to the LLM you selected. Step 6: The LLM generates its response. Step 7: The LLM's response passes through the SAME barrier series (post-validation). Step 8: A new thermosolve signature is computed for the output. Step 9: If all barriers pass, the response is cached as a TEEP and delivered to you with its validation signature. Step 10: If any barrier fails at any step, the output is blocked.",
    "",
    ...
    "",
    "## Multi-Model Consensus (Ensemble Thermosolve)",
    "CPUAGEN can query 2-6 LLM providers simultaneously via the Ensemble Thermosolve system. Each provider's response is thermosolve-validated independently, then a weighted centroid is computed. Outlier detection removes providers that deviate significantly from consensus. The agreement score (0-100%) indicates how tightly the providers cluster. Available in the Dashboard Lab tab.",
    "",
    "## Semantic Cannon",
    "A 3-stage golden-ratio entropy compression pipeline: Stage 1 (Cannon Fire) compresses entropy by phi=0.382, Stage 2 (Cavitation) creates a semantic vacuum via golden collapse, Stage 3 (Mach Diamond) locks flux at dS=0 as a standing wave. Available in the Dashboard Lab tab.",
    "",
    "## Fisher Information Geometry",
    "CPUAGEN uses a 7x7 Fisher information matrix to track correlations between semantic dimensions (entropy, coherence, truth, naturality, thermal stability, synergy). Dimension weights adapt dynamically based on which properties matter most for your queries. The Fisher matrix heatmap is visible in the Dashboard Geometry tab.",
    "",
    "## Riemannian Manifold & Holographic Projection",
    "The enforcement engine tracks queries as trajectories on a 5D semantic manifold. A holographic boundary projection maps 12D TEEP signatures onto 5D coordinates via Fisher-weighted reduction. Ricci curvature analysis identifies dense knowledge regions. Manifold coverage shows how much of the semantic space has been explored.",
    "",
    "## Bekenstein Bound Compression",
    "Every TEEP signature undergoes Bekenstein compression before storage. The Bekenstein bound (S_max = 2πRE/ℏc) sets the maximum information density for a given energy and radius in physics. CPUAGEN applies this principle: signatures are compressed to the theoretical maximum information density, discarding precision below the Bekenstein limit. This ensures optimal storage efficiency while preserving all meaningful semantic content.",
    "",
    "## Mach Diamond Detection",
    "When multiple queries converge on similar semantic regions, the enforcement engine detects standing-wave interference patterns called Mach Diamonds. Named after the shock diamond patterns in supersonic jet exhaust, these indicate where independent query trajectories reinforce each other — revealing high-confidence knowledge regions. Detected diamonds mark semantic convergence points where the system has high certainty.",
    "",
    "## TEEP Chain Tracing",
    "Every TEEP records its causal lineage: which query produced it (parent) and what it generated (children). TEEP Chain Tracing traverses this directed acyclic graph (DAG) in any direction — backward to find root causes, forward to see consequences, or both to map the full causal web. This enables conversation replay, knowledge provenance, and understanding how insights compound over time.",
    "",
    ...
    "- **Dashboard** (/app/dashboard) — Physics exploration interface with 9 tabs: Overview (metrics), TEEPs (inspection), Fisher (7x7 matrix heatmap), PsiState (26-dim gauges), Manifold (holographic scatter), Geometry (Ricci curvature), Enforcement (cache, AGF, morphic, holographic, recent TEEPs, export/import), and Lab (manual thermosolve + Cannon + Ensemble).",
    ...
    "- **Agent Loop** (/app/agent) — Tier 2 multi-turn agent with tool calling (web search, calculator, code execution, file generation, URL fetch, datetime). The agent plans, executes tools, observes results, and iterates until task completion with CBF enforcement at each step.",
    "- **Memory** (/app/memory) — Knowledge base showing enforcement engine state, TEEP history, morphic field strength, and manifold/Fisher geometry.",
    ...
    "Unlike traditional AI chatbots where every question is answered from scratch, CPUAGEN's TEEP system means knowledge compounds over time. The system currently has over 7 million solved basin states.",
    "",
    "# Dual Mode",
    "",
